...
import java.util.function.Function;
        ...
        public void extentSetup(ITestContext context) {
           report.ExtentManager.setOutputDirectory(context);
            extent = report.ExtentManager.getInstance();
        }
        @BeforeMethod
        public void startExtent(Method method) {
            ...
            report.ExtentTestManager.startTest( methodName );
            report.ExtentTestManager.getTest().assignCategory(className);
        }
        ...
        public void afterEachTestMethod(@Optional("") String appName, ITestResult result) {
            report.ExtentTestManager.getTest().getTest().setStartedTime(getTime(result.getStartMillis()));
            report.ExtentTestManager.getTest().getTest().setEndedTime(getTime(result.getEndMillis()));
            for (String group : result.getMethod().getGroups()) {
                report.ExtentTestManager.getTest().assignCategory(group);
            }
            if (result.getStatus() == 1) {
                report.ExtentTestManager.getTest().log(LogStatus.PASS, "Test Passed");
            } else if (result.getStatus() == 2) {
                report.ExtentTestManager.getTest().log(LogStatus.FAIL, getStackTrace(result.getThrowable()));
            } else if (result.getStatus() == 3) {
                report.ExtentTestManager.getTest().log(LogStatus.SKIP, "Test Skipped");
            }
            report.ExtentTestManager.endTest();
            extent.flush();
